...
class SimpleFCNet(nn.Module):
    def __init__(self, categorical, numeric, categorical_dict):
        ...
        self.convolution = nn.Sequential(
            nn.Conv1d(1, 100, kernel_size=4),
            nn.Conv1d(100, 80, kernel_size=4),
            nn.MaxPool1d(3, stride=2),
            nn.Dropout(p=0.2)
        )
        self.convolution = nn.DataParallel(self.convolution)
    ...
    def forward(self, x):
        ...
        x = self.dropout1(x)
        x = x.view(current_bs, 1, -1)
        x = self.convolution(x)
        x = x.view(current_bs, -1)
        x = self.regression(x)
        return x
