...
class GRUNet(nn.Module):
    def __init__(self, categorical_dict, batch_size, numeric, word_embed, rnn_input=300, hidden_state=64, hidden_layers=1):
        '''
        ...
        self.lstm = nn.LSTM(rnn_input, hidden_state, hidden_layers).cuda()
    ...
    def init_hidden(self):
        self.hidden = torch.zeros(self.hidden_layers, 100, self.hidden_state).cuda()
        self.c = torch.zeros(self.hidden_layers, 100, self.hidden_state).cuda()
    ...
    def forward(self, x_cat, x_num, x_words):
        ...
        x_words, (self.hidden, self.c) = self.lstm(x_words, (self.hidden, self.c))
